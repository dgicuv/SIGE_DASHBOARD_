...
export type ChartSeries = { name: string; values: readonly number[] };
...
type UseChartParams = {
    ...
    /** Series múltiples (bar/line) alineadas a `categories`. Si se define, sustituye a `values` en la gráfica. No aplica a chartType="pie". */
    series?: ChartSeries[];
    selectedSex?: string;
    selectedYear?: string;
...
export function useChart({
    ...
    const hasMultiSeries = chartType !== "pie" && !!series && series.length > 0;
    ...
    const option = useMemo<EChartsOption>(
        () => {
            ...
            const labelFormatter = (params: { value: number }) =>
                formatValue === "percent"
                    ? `${total > 0 ? Math.round((params.value / total) * 100) : 0}%`
                    : params.value.toLocaleString();

            const seriesList = hasMultiSeries
                ? series!.map((s) => ({
                    name: s.name,
                    type: chartType,
                    data: [...s.values],
            ...
            return {
                aria: chartType === "bar" ? {enabled: true, decal: {show: true}} : undefined,
                tooltip: {
                    trigger: "axis",
                    show: true,
                    formatter: (params: unknown) => {
                        ...
                        if (hasMultiSeries) {
                            const categoryTotal = arr.reduce((sum, p) => sum + p.value, 0);
                            const lines = arr.map((p) => {
                                const percent = categoryTotal > 0 ? Math.round((p.value / categoryTotal) * 100) : 0;
                                return formatValue === "percent"
                                    ? `${p.seriesName}: ${percent}% (${p.value.toLocaleString()})`
                                    : `${p.seriesName}: ${p.value.toLocaleString()} (${percent}%)`;
                            });
                        ...
                        return formatValue === "percent"
                            ? `${p.name}: ${percent}% (${p.value.toLocaleString()})`
                            : `${p.name}: ${p.value.toLocaleString()} (${percent}%)`;
                    },
                },
                color: colors,
                title: title
                    ...
                    : undefined,
                grid: {left: 40, right: 40, top: 64, bottom: hasMultiSeries ? 64 : 44},
    ...
    const exportExtra: EChartsOption = chartType === "pie"
        ? {
            title: {
                text: title,
        ...
        : {
            ...
            ...(hasMultiSeries ? {legend: {textStyle: {color: exportLabelColor}}} : {}),
            series: [{label: {color: exportLabelColor}}],
        };
